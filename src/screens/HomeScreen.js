...
export default function HomeScreen() {
  ...
  // Definição da Paleta de Cores (Cores fornecidas pelo Pedro)
  const lavanda = '#9e86bd'; // Lavanda para fundo ou detalhes
  const roxoProfundo = '#4d235e'; // Roxo Profundo para botões e textos principais
  ...
  return (
    <ScrollView className="flex-1 bg-white p-6">
      ...
      {/* CABEÇALHO */}
      <View className="mb-8">
        <Text className="text-3xl font-extrabold" style={{ color: roxoProfundo }}>Bem-vinda, Pietra!</Text>
        <Text className="text-sm font-medium text-gray-500 mt-1 italic">Empodere sua jornada empreendedora com a Annik.</Text>
      </View>

      {/* BOX PRINCIPAL: PLANO DE NEGÓCIO (Se baseando na estrutura da imagem) */}
      <TouchableOpacity 
      ...
      {/* GRID DE BOTÕES (2 Colunas, estruturalmente similar à imagem) */}
      <View className="flex-row justify-between mb-16">
        
        {/* COLUNA ESQUERDA */}
        <View className="w-[48%] gap-4">
          
          {/* BOTÃO 1: PRECIFICAÇÃO (Vai para ConfiguracaoNegocio) */}
          <TouchableOpacity 
            className="p-6 rounded-[30px] shadow-lg flex-col justify-between" 
            style={{ backgroundColor: lavanda }}
            onPress={() => navigation.navigate('ConfiguracaoNegocio')} // Navegação para ConfiguracaoNegocio
          >
            <Text className="text-3xl mb-3">💸</Text>
            <Text className="font-extrabold text-white text-lg">Precificação</Text>
          </TouchableOpacity>

          {/* BOTÃO 2: GERAR RELATÓRIOS (Placeholder) */}
          <TouchableOpacity 
            className="p-6 rounded-[30px] shadow-lg flex-col justify-between" 
            style={{ backgroundColor: lavanda }}
            onPress={() => console.log('Gerar relatórios pressionado')}
          >
            <Text className="text-3xl mb-3">📈</Text>
        ...
        <View className="w-[48%] gap-4">
          ...
          {/* BOTÃO 3: PERSONALIZAÇÃO (Placeholder) */}
          <TouchableOpacity 
            className="p-6 rounded-[30px] shadow-lg flex-col justify-between" 
          ...
          {/* BOTÃO 4: CONFIGURAÇÕES (Placeholder para completar o grid) */}
          <TouchableOpacity 
            className="p-6 rounded-[30px] shadow-lg flex-col justify-between" 
            style={{ backgroundColor: roxoProfundo }}
